...
def child(conn):
	global slot1, slot2, activeSlot,recoil11,recoil12,recoil21,recoil22, gameselected, processfound, rcsactive, rapidFire,overlaykey, overlayActive
	while True:
		try:
			msg = conn.recv()
			setting, weaponcfg = msg.split("|")
			processfound, gameselected, rcsactive, rapidFire, overlaykey, overlayActive = setting.split(";")
...
def updateGui(hWnd):
	if overlayActive.lower() == "True".lower() or overlayActive == True:
		...
		win32gui.DrawText(hdc,'Rapidfire: ' + "{:<5}".format(str(rapidFire)),-1,(0,0,int(rect[2]*0.98),int((rect[3]+300)*0.519)),win32con.DT_RIGHT | win32con.DT_BOTTOM | win32con.DT_NOCLIP | win32con.DT_SINGLELINE)
		win32gui.DrawText(hdc,'Toggle Overlay: ' + "{:<5}".format(str(overlaykey)),-1,(0,0,int(rect[2]*0.98),int((rect[3]+350)*0.519)),win32con.DT_RIGHT | win32con.DT_BOTTOM | win32con.DT_NOCLIP | win32con.DT_SINGLELINE)
